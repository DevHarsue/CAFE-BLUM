...
from conexion.tablas import TablaDivisas
...
class VistaDivisas:
    ...
    def cargar_divisa(self):
        ...
        self.ui.double_valor_adivisa.setValue(self.divisa[-1])
        if self.indice==1:
    ...
    def actualizar_divisa(self):
        ...
        self.ventana.vista_factura.tasa_bolivares = float(TablaDivisas().select(1)[0][-1])
        self.ventana.vista_factura.tasa_cop = float(TablaDivisas().select(2)[0][-1])
